...
class WeightsEnhancementStep(PipelineStep):
    ...
    def __init__(self, config: ReweightingConfig, logger: logging.Logger, lotus_store: LotusStore):
    ...
    def process(self, analysis: Analysis) -> Analysis:
        enhancer = WeightsEnhancer(self.config, self.logger, self.lotus_store)
        try:
